...
const NormalUserPortal = () => {
  ...
  const handleSendMessage = () => {
    ...
    // Check if profile completion is required and show prompt if needed
    if (shouldShowProfilePrompt && !isProfileComplete) {
      uiToast({
        title: "Complete Your Profile",
        description: "Please complete your profile to continue using the service.",
        duration: 5000,
      });
      setShowProfileDialog(true);
      return;
    ...
    const newCount = userChatCount + 1;
    setUserChatCount(newCount);
    localStorage.setItem("userChatCount", newCount.toString());

    // Simulate bot response after a delay
    setTimeout(() => {
      const botResponse = generateBotResponse(message, currentLanguage);
